...
private:
  ...
  // Remote memory classification: region-level flag.
  // Per-object classification is in mark word bits 39-40 (primary).
  // This flag is a fast-negative test: if false, NO objects in this region
  // are classified, so write barrier can skip mark word check entirely.
...
public:
  ...
  // Region-level classification flag.
  // Per-object classification is in mark word bits 39-40 (markWord::remote_class_*).
  ...
  bool _rss_trimmed_free;     // Free-region pages have already been madvise'd
  ...
  bool is_rss_trimmed_free() const  { return _rss_trimmed_free; }
  void set_rss_trimmed_free()       { _rss_trimmed_free = true; }
  void clear_rss_trimmed_free()     { _rss_trimmed_free = false; }
