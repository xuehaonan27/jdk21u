...
#ifdef XHN_EVAC_RC
  _old_task_queues = new G1ScannerTasksQueueSet(n_queues);

  for (uint i = 0; i < n_queues; i++) {
    G1ScannerTasksQueue* q = new G1ScannerTasksQueue();
    _old_task_queues->register_queue(i, q);
  }
#endif // XHN_EVAC_RC
...
#ifdef XHN_EVAC_RC
  guarantee(_old_task_queues != nullptr, "old_task_queues allocation failure.");
#endif // XHN_EVAC_RC
}
